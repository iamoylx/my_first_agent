...
async def generate_image(prompt: str, negative_prompt: str = '',
    ...
    # 探测 Forge 是否可达；不可达时自动回退 Agnes 云端生图（对 agent 透明，
    # 保证"生成图片"总能出图，不再因为本地服务没启动而失败）
    forge_ok = False
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(f'{FORGE_BASE}/sdapi/v1/sd-models', timeout=aiohttp.ClientTimeout(total=3)) as resp:
                if resp.status == 200:
                    forge_ok = True
    except Exception:
        forge_ok = False
    if not forge_ok:
        try:
            ...
            agnes_prompt = prompt.strip()
            return await _agnes_gen(prompt=agnes_prompt, size=f'{width}x{height}')
...
TOOLS = [{
    ...
    'function': {
        ...
        'description': '生成图片：优先走本地 Stable Diffusion（WebUI Forge，隐私不出本机）；本地服务未启动时自动改用 Agnes 云端生成，保证总能出图。',
        'parameters': {
            'type': 'object',
            'properties': {
